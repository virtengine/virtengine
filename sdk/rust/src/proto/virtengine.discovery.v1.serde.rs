...
impl serde::Serialize for VirtEngine {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("virtengine.discovery.v1.VirtEngine", len)?;
...
impl<'de> serde::Deserialize<'de> for VirtEngine {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = VirtEngine;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct virtengine.discovery.v1.VirtEngine")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<VirtEngine, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
        ...
        deserializer.deserialize_struct("virtengine.discovery.v1.VirtEngine", FIELDS, GeneratedVisitor)
    }
}
