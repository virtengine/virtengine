...
func (s *AuthzCLITestSuite) SetupSuite() {
	...
	_, err := govclitestutil.MsgSubmitLegacyProposal(s.cctx, val[0].Address.String(),
		...
		fmt.Sprintf("--%s=%s", cflags.FlagDeposit, sdk.NewCoin("uve", govv1.DefaultMinDepositTokens).String()))
	...
	out, err := clitestutil.ExecSend(
		...
		cli.TestFlags().
			With(
				...
				sdk.NewCoins(
					sdk.NewCoin("uve", sdkmath.NewInt(200)),
				).String(),
			).
			WithSkipConfirm().
			WithBroadcastModeSync().
...
func (s *AuthzCLITestSuite) TestCLITxGrantDepositAuthorization() {
	...
	testCases := []struct {
		...
		{
			"Valid tx deposit authorization (uve)",
			cli.TestFlags().
				With(
					grantee.String(),
					"deposit",
...
func (s *AuthzCLITestSuite) TestExecSendAuthzWithAllowList() {
	...
	tokens := sdk.NewCoins(
		sdk.NewCoin("uve", sdkmath.NewInt(12)),
	)
