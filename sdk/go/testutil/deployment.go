...
func DeploymentGroups(t testing.TB, did dtypes.DeploymentID, gseq uint32) dvbeta.Groups {
	t.Helper()
	count := rand.Intn(5) + 5 //nolint:gosec // G404: test helper randomness is non-security-sensitive
	vals := make(dvbeta.Groups, 0, count)
	for i := 0; i < count; i++ {
		vals = append(vals, DeploymentGroup(t, did, gseq+uint32(i))) //nolint:gosec // G404: test helper randomness is non-security-sensitive
	}
	return vals
}
