...
func ReadFile(path string) (SDL, error) {
	buf, err := os.ReadFile(path) /* #nosec G304 -- path is a local path parameter supplied by this function's own caller (a CLI argument, loader parameter or configured state file) and is not derived from a network peer or chain message; opening the caller-nominated file is the purpose of this call */ //nolint:gosec
	if err != nil {
		return nil, err
	}
	return Read(buf)
