...
Deterministic local fixtures cover exact quote math, stale/invalid evidence, side-effect boundaries, signed observations and reconciliation contracts. They do not prove that any declared Osmosis testnet pool or payout-provider sandbox was contacted. Real testnet/sandbox conformance remains required before certification, and real production execution remains required before a production row can advance.
...
| DEX-OSMOSIS-TESTNET-V1 | External testnet conformance | Osmosis testnet / `osmo-test-5` | Osmosis / `gamm-v1beta1-cp-equal-weight-v1` | Pool IDs `TBD_EXTERNAL`; no checked-in executable profile | Token denoms/decimals `TBD_EXTERNAL` | Profile limits and retained real testnet execution/finality evidence `TBD_EXTERNAL` | Test oracle/custody evidence `TBD_EXTERNAL` | `engineering_complete_external_blocked` |
...
The production-capable code supports only two-asset, equal-weight constant-product GAMM pools. It verifies exact reserve direction by denom, local constant-product exact-in math, fees, minimum output, price impact, oracle deviation, pool state at a known height/block, quote digest/expiry, canonical payload binding, actual Cosmos `TxRaw`, confirmed output and profile-defined finality. Provider-daemon construction requires one authenticated bound pool provider whose payload, response height, block hash and source node identity agree with independent chain evidence; a plain unheighted LCD response is not executable. Production service registration also requires an independently authorized exact profile.
...
| PAYOUT-SANDBOX-JSON-V1 | External sandbox conformance | Sandbox partner and exact API version `TBD_EXTERNAL` | Test jurisdiction/currency/rail `TBD_EXTERNAL` | Same strict profile schema; sandbox decisions/beneficiary tokens `TBD_EXTERNAL` | Real sandbox credential, webhook and conformance evidence `TBD_EXTERNAL` | No checked-in sandbox profile, credentials or execution report | `engineering_complete_external_blocked` |
