...
const tests = [
  ["accepts an annotated handoff tag binding the payload", () => { const entry = { thread: "T1", tag: "checkpoint/prototype-t1/t1-09", payload_sha: sha("c") }; const handoff = sha("d"); const runGit = (_repo, args) => args[0] === "cat-file" && args[1] === "-t" ? { status: 0, stdout: "tag\n" } : args[0] === "cat-file" ? { status: 0, stdout: `object ${handoff}\ntype commit\ntag ${entry.tag}\n` } : args[0] === "rev-parse" ? { status: 0, stdout: `${handoff}\n` } : args[0] === "show" ? { status: 0, stdout: JSON.stringify({ thread: "T1", payload_head: entry.payload_sha }) } : { status: 0, stdout: "" }; assert.equal(verifyAcceptedPayload(".", entry, runGit), true); }],
  ["rejects a tag peeling directly to the payload", () => { const entry = { thread: "T1", tag: "checkpoint/prototype-t1/t1-09", payload_sha: sha("c") }; const runGit = (_repo, args) => args[0] === "cat-file" && args[1] === "-t" ? { status: 0, stdout: "tag\n" } : args[0] === "cat-file" ? { status: 0, stdout: `object ${entry.payload_sha}\ntype commit\ntag ${entry.tag}\n` } : { status: 0, stdout: `${entry.payload_sha}\n` }; assert.equal(verifyAcceptedPayload(".", entry, runGit), false); }],
  ["rejects a lightweight tag", () => { const entry = { thread: "T1", tag: "checkpoint/prototype-t1/t1-09", payload_sha: sha("c") }; const runGit = () => ({ status: 0, stdout: "commit\n" }); assert.equal(verifyAcceptedPayload(".", entry, runGit), false); }],
  ["rejects a handoff binding another payload", () => { const entry = { thread: "T1", tag: "checkpoint/prototype-t1/t1-09", payload_sha: sha("c") }; const handoff = sha("d"); const runGit = (_repo, args) => args[0] === "cat-file" && args[1] === "-t" ? { status: 0, stdout: "tag\n" } : args[0] === "cat-file" ? { status: 0, stdout: `object ${handoff}\ntype commit\ntag ${entry.tag}\n` } : args[0] === "rev-parse" ? { status: 0, stdout: `${handoff}\n` } : args[0] === "show" ? { status: 0, stdout: JSON.stringify({ thread: "T1", payload_head: sha("e") }) } : { status: 0, stdout: "" }; assert.equal(verifyAcceptedPayload(".", entry, runGit), false); }],
  ["rejects a non-descendant handoff target", () => { const entry = { thread: "T1", tag: "checkpoint/prototype-t1/t1-09", payload_sha: sha("c") }; const handoff = sha("d"); const runGit = (_repo, args) => args[0] === "cat-file" && args[1] === "-t" ? { status: 0, stdout: "tag\n" } : args[0] === "cat-file" ? { status: 0, stdout: `object ${handoff}\ntype commit\ntag ${entry.tag}\n` } : args[0] === "rev-parse" ? { status: 0, stdout: `${handoff}\n` } : args[0] === "show" ? { status: 0, stdout: JSON.stringify({ thread: "T1", payload_head: entry.payload_sha }) } : { status: 1, stdout: "" }; assert.equal(verifyAcceptedPayload(".", entry, runGit), false); }],
  ["rejects a missing or malformed committed handoff", () => { const entry = { thread: "T1", tag: "checkpoint/prototype-t1/t1-09", payload_sha: sha("c") }; const handoff = sha("d"); const base = (_repo, args) => args[0] === "cat-file" && args[1] === "-t" ? { status: 0, stdout: "tag\n" } : args[0] === "cat-file" ? { status: 0, stdout: `object ${handoff}\ntype commit\ntag ${entry.tag}\n` } : args[0] === "rev-parse" ? { status: 0, stdout: `${handoff}\n` } : { status: 1, stdout: "" }; assert.equal(verifyAcceptedPayload(".", entry, base), false); const malformed = (_repo, args) => args[0] === "show" ? { status: 0, stdout: "not-json" } : base(_repo, args); assert.equal(verifyAcceptedPayload(".", entry, malformed), false); }],
