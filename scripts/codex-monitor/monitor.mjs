...
// ── Preflight checks ─────────────────────────────────────────────────────────
let preflightPassed = !preflightEnabled;
let preflightRetryTimeout = null;
...
async function runPreflight() {
  ...
  console.log("[monitor] running preflight checks...");
  const report = await runPreflightChecks(repoRoot);
  if (report.passed) {
    console.log("[monitor] ✓ all preflight checks passed");
    preflightPassed = true;
    return true;
  }
  const formatted = formatPreflightReport(report);
  console.error(`[monitor] ✗ preflight checks failed:\n${formatted}`);
  if (telegramToken && telegramChatId) {
    void sendTelegramMessage(
      `⚠️ Preflight checks failed. Orchestrator blocked until resolved.\n\n${formatted}`,
    );
  }
  preflightPassed = false;
  const retryMsg = `[monitor] retrying preflight in ${Math.round(preflightRetryMs / 1000)}s`;
  console.log(retryMsg);
  preflightRetryTimeout = setTimeout(() => {
    void runPreflight().then((ok) => {
      if (ok && !currentChild) startProcess();
    });
  }, preflightRetryMs);
  return false;
}

void runPreflight().then((passed) => {
  if (passed) startProcess();
});
