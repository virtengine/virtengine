...
1. **NO_CHANGES infinite loop** ✅ FIXED  
   - **Root cause:** orchestrator asked agents to reply "NO_CHANGES" if no work needed, but never parsed the response, so fresh tasks looped.  
   - **Fix:** `Get-AttemptFailureCategory` now detects NO_CHANGES and immediately archives; task descriptions with "superseded by", "already completed", etc. are detected and skipped.  
   - References: `ve-orchestrator.ps1:L2449-L2452`, `ve-orchestrator.ps1:L3563-L3580`, `ve-orchestrator.ps1:L5206-L5229`

2. **Already-merged task infinite retry loop** ✅ FIXED  
   - **Root cause:** completed/merged tasks hit the "no remote branch" retry path without checking if the work was already merged.  
   - **Fix (orchestrator):** `Test-BranchMergedIntoBase` + `Test-TaskDescriptionAlreadyComplete` run before retry logic; already-merged branches bypass manual review.  
   - **Fix (follow-up cap):** `$script:TaskFollowUpCounts` is wired to stop repeated follow-ups after `MAX_FOLLOWUPS_PER_TASK`.  
   - **Fix (monitor.mjs):** `resolveAndTriggerSmartPR` and `smartPRFlow` check merged-branch cache and completion signals before work.  
   - References: `ve-orchestrator.ps1` — `Test-BranchMergedIntoBase`, `Test-TaskDescriptionAlreadyComplete`, `Try-SendFollowUp`, `Try-SendFollowUpNewSession`; `monitor.mjs` — `resolveAndTriggerSmartPR`, `smartPRFlow`
