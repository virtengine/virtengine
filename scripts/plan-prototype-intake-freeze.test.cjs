...
const tests = [
  ...
  ["rejects a late tag", () => assert.throws(() => planFrozenEpoch(epoch(), new Map([["T1", "checkpoint/prototype-t1/t1-09"]]), { now: Date.parse("2000-01-03T00:00:00Z"), observation: observation(), resolveTag: () => ({ target: "c".repeat(40), tagger_at: "2000-01-02T00:00:01Z" }) }), /after the announcement cutoff/)],
  ["rejects a tag published before the epoch opened", () => assert.throws(() => planFrozenEpoch(epoch(), new Map([["T1", "checkpoint/prototype-t1/t1-09"]]), { now: Date.parse("2000-01-03T00:00:00Z"), observation: observation(), resolveTag: () => ({ target: "c".repeat(40), tagger_at: "1999-12-31T23:59:59Z" }) }), /before the epoch opened/)],
  ...
  ["rejects an invalid target", () => assert.throws(() => planFrozenEpoch(epoch(), new Map([["T1", "checkpoint/prototype-t1/t1-09"]]), { now: Date.parse("2000-01-03T00:00:00Z"), observation: observation(), resolveTag: () => ({ target: "HEAD", tagger_at: "2000-01-01T12:00:00Z" }) }), /not a commit SHA/)],
