...
function evidence() {
  const observation = { schema_version: "virtengine.prototype.intake-tag-observation/v1", intake_epoch: 1, announcement_cutoff: "2000-01-02T00:00:00Z", observed_at: "2000-01-01T18:00:00Z", tags: [{ thread: "T1", tag: "checkpoint/prototype-t1/t1-09", tag_object: "f".repeat(40), target: "c".repeat(40) }] };
...
const tests = [
  ...
  ["removes the temporary file when atomic replacement fails", () => { const calls = []; const operations = { openSync: () => { calls.push("open"); return 7; }, writeFileSync: () => calls.push("write"), fsyncSync: () => calls.push("fsync"), closeSync: () => calls.push("close"), renameSync: () => { calls.push("rename"); throw new Error("rename failed"); }, unlinkSync: () => calls.push("unlink") }; assert.throws(() => atomicWriteFile("epoch.json", "frozen\n", operations, "epoch.json.tmp"), /rename failed/); assert.deepEqual(calls, ["open", "write", "fsync", "close", "rename", "unlink"]); }],
  ["closes and removes a partially written temporary file", () => { const calls = []; const operations = { openSync: () => { calls.push("open"); return 7; }, writeFileSync: () => { calls.push("write"); throw new Error("partial write"); }, fsyncSync: () => calls.push("fsync"), closeSync: () => calls.push("close"), renameSync: () => calls.push("rename"), unlinkSync: () => calls.push("unlink") }; assert.throws(() => atomicWriteFile("epoch.json", "frozen\n", operations, "epoch.json.tmp"), /partial write/); assert.deepEqual(calls, ["open", "write", "close", "unlink"]); }],
  ...
  ["accepts a plan backed by observed tag evidence", () => { const value = evidence(); assert.equal(validateFreezeEvidence(epoch(), frozen(), value.content, "observation.json", value.manifest, { now: afterCutoff, sourceContent: value.content, sourceIsAncestor: true, resolveTag: () => ({ tag_object: "f".repeat(40), target: "c".repeat(40), tagger_at: "2000-01-01T12:00:00Z" }) }), true); }],
  ["rejects an observation omitting an in-window tag during application replay", () => { const value = evidence(); const observation = JSON.parse(value.content); observation.tags = []; const content = JSON.stringify(observation); value.manifest.control_artifacts[0].sha256 = createHash("sha256").update(content).digest("hex"); const currentTags = [{ thread: "T1", tag: "checkpoint/prototype-t1/t1-09", tag_object: "f".repeat(40), target: "c".repeat(40) }]; assert.throws(() => validateFreezeEvidence(epoch(), frozen(), content, "observation.json", value.manifest, { now: afterCutoff, sourceContent: content, sourceIsAncestor: true, currentTags, resolveTag: () => ({ tag_object: "f".repeat(40), target: "c".repeat(40), tagger_at: "2000-01-01T12:00:00Z" }) }), /omitted from the in-window tag observation/); }],
  ["rejects an announced tag absent from observation evidence", () => { const value = evidence(); const plan = frozen(); plan.producers[0].tag = "checkpoint/prototype-t1/t1-10"; assert.throws(() => validateFreezeEvidence(epoch(), plan, value.content, "observation.json", value.manifest, { now: afterCutoff, sourceContent: value.content, sourceIsAncestor: true, resolveTag: () => ({ tag_object: "f".repeat(40), target: "e".repeat(40), tagger_at: "2000-01-01T12:00:00Z" }) }), /not uniquely observed/); }],
