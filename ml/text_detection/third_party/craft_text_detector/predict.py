...
def get_prediction(
    ...
    Output:
        {"masks": lists of predicted masks 2d as bool array,
         ...
         "score_maps": raw float32 region/link score maps in [0, 1] (VirtEngine addition),
    ...
    return {
        ...
        # VirtEngine addition: the raw (non-colourised) float32 score maps that
        # `image_utils.cvt2HeatmapImg` turns into the visualisations above. They keep
        # the network's native heatmap resolution (half the resized input), so callers
        # that need original-image coordinates must rescale by
        # (img_width / score_map_width, img_height / score_map_height).
        "score_maps": {
            "text_score_map": score_text,
            "link_score_map": score_link,
        },
        "times": times,
    }
