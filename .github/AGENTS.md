...
| `standardize-yaml.yaml` | Enforce .yaml | push/pr main paths **/*.yml | `check-yml-files` | Fail on tracked .yml list | git + bash |
...
**Symptom:** `security.yaml` fails with "CodeQL: Advanced setup required"`
**Fix:** See `_docs/operations/ci-troubleshooting.md:44` for language-specific configuration
