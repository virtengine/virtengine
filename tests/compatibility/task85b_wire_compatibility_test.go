...
func TestTask85BAdditiveFieldNumbers(t *testing.T) {
	// Numbers in this test are derived from the generated struct tags, which are
	// the wire contract. The merge 42dff4d5b (Jul 28) reconciled two divergent
	// field-number claims by keeping each block contiguous: it inserted
	// last_error_retryable and shifted the fields that followed. It regenerated
	// the Go/TS/descriptor artifacts but did not update this assertion, so the
	// expectation here had been stale since that merge. Every number below is
	// pinned (including the bookkeeping fields that bracketed the shift) so a
	// future renumber trips the contract instead of sliding through.
	assertFieldNumbers(t, reflect.TypeOf(settlementv1.PayoutRecord{}), map[string]int{
		...
		"external_finality_hash": 28, "value_movement_applied": 29,
		"value_movement_effect_hash": 30,
	})
