...
var (
	...
	upgradeVersion = flag.String("upgrade-version", "local", "virtengine release to download. local if it is built locally")
...
func TestUpgrade(t *testing.T) {
	...
	*sourcesdir = strings.TrimSuffix(*sourcesdir, "/")
	...
	for idx, name := range cfg.Validators {
		...
		genesisBin := fmt.Sprintf("%s/cosmovisor/genesis/bin/virtengine", homedir)

		info, err = os.Stat(genesisBin)
		require.NoError(t, err)
		require.False(t, info.IsDir(), "value in genesis-binary flag is not a file")
		require.True(t, isOwnerExecutable(info.Mode()), "virtengine must be executable file")
		...
		valCmd := &commander{
			...
			env: []string{
				...
				fmt.Sprintf("VE_GAS_PRICES=0.0025uve"),
				fmt.Sprintf("VE_GAS_ADJUSTMENT=2"),
				fmt.Sprintf("VE_P2P_PEX=false"),
				fmt.Sprintf("VE_MINIMUM_GAS_PRICES=0.0025uve"),
