...
func generateMockValidator(index int) *MockValidator {
	v := &MockValidator{
		...
		Commission:      uint16(500 + index%1000), // 5-15%
	...
	h := sha256.New()
	h.Write([]byte(fmt.Sprintf("validator_%d", index)))
	sum := h.Sum(nil)
...
func TestValidatorScaleBaseline(t *testing.T) {
	...
	memPerValidator := int64(m.HeapAlloc) / int64(scale)
...
func TestMemoryPressureAtScale(t *testing.T) {
	...
	for _, scale := range scales {
		t.Run(fmt.Sprintf("scale_%d", scale), func(t *testing.T) {
			...
			memPerValidator := memUsed / uint64(scale)
