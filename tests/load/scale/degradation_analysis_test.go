	...
	for _, scale := range scales {
		t.Run(fmt.Sprintf("scale_%d", scale), func(t *testing.T) {
			...
			analyzer.RecordMetric(PerformanceMetric{
				...
				MemoryUsedMB:   safeInt64FromUint64((memAfter - memBefore) / 1024 / 1024),
...
func TestMarketplaceScaleDegradation(t *testing.T) {
	...
	for _, scale := range scales {
		t.Run(fmt.Sprintf("scale_%d", scale), func(t *testing.T) {
			...
			start = time.Now()
			for i := 0; i < 100; i++ {
				orderID := safeUint64FromInt(t, i%scale+1)
				_, _ = store.SubmitBid(orderID, generateRandomAddress(), 500)
			}
...
func TestMemoryDegradation(t *testing.T) {
	...
	for _, scale := range scales {
		t.Run(fmt.Sprintf("scale_%d", scale), func(t *testing.T) {
			...
			scaleMultiplier := safeUint64FromInt(t, scale*2)
			perEntry := (after.HeapAlloc - before.HeapAlloc) / scaleMultiplier
...
func safeUint64FromInt(t *testing.T, value int) uint64 {
	t.Helper()
	if value < 0 {
		t.Fatalf("expected non-negative value, got %d", value)
	}
	return uint64(value)
}
