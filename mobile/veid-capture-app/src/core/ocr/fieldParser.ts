...
export function parseOcrFields(rawText: string): ParsedOcrResult {
  ...
  if (nameMatch?.[1]) {
    fields.push({ key: "name", value: nameMatch[1].trim(), confidence: unavailableConfidence });
  }

  const surnameMatch = normalized.match(/SURNAME[:\s]+([A-Z\s]+)/);
  if (surnameMatch?.[1]) {
    fields.push({ key: "surname", value: surnameMatch[1].trim(), confidence: unavailableConfidence });
  }

  const dob = findFirstMatch(normalized, datePattern);
  if (dob) {
    fields.push({ key: "date_of_birth", value: dob, confidence: unavailableConfidence });
  }

  const expiryMatch = normalized.match(/EXPIRY[:\s]+(\d{2}[\/\-.]\d{2}[\/\-.]\d{4})/);
  if (expiryMatch?.[1]) {
    fields.push({ key: "expiry", value: expiryMatch[1], confidence: unavailableConfidence });
  }

  const docId = findFirstMatch(normalized, documentIdPattern);
  if (docId) {
    fields.push({ key: "document_number", value: docId, confidence: unavailableConfidence });
  }
