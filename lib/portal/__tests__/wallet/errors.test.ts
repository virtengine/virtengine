...
describe('WalletError', () => {
  describe('WalletError class creation', () => {
    it('should create error with default message from code', () => {
      ...
      expect(error.message).toBe(WALLET_ERROR_MESSAGES[WalletErrorCode.WALLET_NOT_INSTALLED]);
    });

    it('should create error with custom message', () => {
      const customMessage = 'Custom error message';
      const error = new WalletError(WalletErrorCode.WALLET_LOCKED, customMessage);
    ...
    it('should set isRetryable based on error code by default', () => {
      ...
      const nonRetryableError = new WalletError(WalletErrorCode.WALLET_NOT_INSTALLED);
    ...
    it('should allow overriding isRetryable', () => {
      const error = new WalletError(WalletErrorCode.WALLET_NOT_INSTALLED, undefined, {
        isRetryable: true,
      });
    ...
    it('should set suggestedAction based on error code by default', () => {
      ...
      expect(error.suggestedAction).toBe('Transfer funds to your account before proceeding.');
    });
    ...
    it('should serialize to JSON correctly', () => {
      ...
      const error = new WalletError(WalletErrorCode.BROADCAST_FAILED, 'Test message', {
        cause,
      });
  ...
  describe('error code mapping', () => {
    ...
    it('should return correct action via getSuggestedAction', () => {
      expect(getSuggestedAction(WalletErrorCode.WALLET_CONNECTION_REJECTED)).toBe(
        'Click Connect and approve in your wallet popup.'
      );
    });
  });
  ...
  describe('withWalletTimeout', () => {
    ...
    it('should propagate original error if promise rejects before timeout', async () => {
      ...
      await expect(withWalletTimeout(failingPromise, 1000)).rejects.toBe(originalError);
    });
