...
const rules = [
  ...
  {
    id: 'onclick-keyboard',
    description: 'onClick handlers on non-interactive elements need keyboard support',
    pattern: /<(?:div|span|p|section|article)[^>]*onClick\s*=/gi,
    message: 'Non-interactive elements with onClick should have onKeyDown/onKeyUp and role',
    severity: 'warning',
  },
  {
  ...
  },
  {
    ...
    message: 'Ensure aria-labelledby/describedby reference existing element IDs',
    severity: 'info',
  },
  {
    id: 'heading-order',
  ...
  },
  {
    ...
    pattern: /role\s*=\s*["'](?:button|link|checkbox|radio|tab|menuitem)[^>]*(?!.*(?:onKeyDown|onKeyUp|onKeyPress))/gi,
    message: 'Elements with interactive roles should have keyboard event handlers',
    severity: 'warning',
  },
  {
    id: 'color-contrast-hardcoded',
    description: 'Check for potentially low-contrast color combinations',
    pattern: /(?:color|backgroundColor)\s*[:=]\s*["']?#(?:999|aaa|bbb|ccc|ddd|eee|777|888)/gi,
    message: 'Light gray colors may have insufficient contrast - verify manually',
    severity: 'info',
  },
  {
    id: 'focus-visible',
...
async function main() {
  ...
  const pattern = args[0] || 'components/**/*.tsx';
