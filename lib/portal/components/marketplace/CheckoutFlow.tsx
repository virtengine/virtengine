...
export function CheckoutFlow({
  ...
  React.useEffect(() => {
    ...
    offering.pricing.depositRequired,
  ...
  const handleConfirm = async () => {
    if (inFlight.current || !mutationAdapter || !resultProjector || !request) return;
  ...
  return (
    <div className={className}>
      {step === 'review' && (
        <div>
          <h4 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: 600 }}>Order Summary</h4>
          ...
          >
            <input
              type="checkbox"
              aria-label="Agree to checkout terms"
              checked={agreed}
      ...
      {step === 'error' && (
        <div role="alert" style={{ textAlign: 'center', padding: '24px' }}>
          <p style={{ margin: '0 0 16px', fontWeight: 500 }}>Order was not committed.</p>
      ...
      {!available && step !== 'processing' && (
        <p role="status" style={{ marginTop: '12px', color: '#666' }}>
          Order placement is unavailable because no authoritative signing and broadcast adapter is
          configured.
        </p>
      )}
    </div>
