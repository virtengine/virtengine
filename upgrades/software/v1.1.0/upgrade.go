...
func (up *upgrade) closeOverdrawnEscrowAccounts(ctx sdk.Context) error {
	...
	for ; iter.Valid(); iter.Next() {
		...
		hooks := mhooks.New(up.Keepers.VirtEngine.Deployment, up.Keepers.VirtEngine.Market)
	...
	for ; biter.Valid(); biter.Next() {
		...
		deployment, found := up.Keepers.VirtEngine.Deployment.GetDeployment(ctx, did)
		if !found {
			return nil
		}
...
func (up *upgrade) OnEscrowAccountClosed(ctx sdk.Context, obj etypes.Account) error {
	...
	deployment, found := up.Keepers.VirtEngine.Deployment.GetDeployment(ctx, id)
	if !found {
		return nil
	}
	...
	err = up.Keepers.VirtEngine.Deployment.CloseDeployment(ctx, deployment)
	if err != nil {
		return err
	}
	...
	for _, group := range up.Keepers.VirtEngine.Deployment.GetGroups(ctx, deployment.ID) {
		if group.ValidateClosable() == nil {
			err = up.Keepers.VirtEngine.Deployment.OnCloseGroup(ctx, group, gstate)
			if err != nil {
				return err
			}
...
func (up *upgrade) OnGroupClosed(ctx sdk.Context, id dv1.GroupID) error {
	processClose := func(ctx sdk.Context, bid mtypes.Bid) error {
		err := up.Keepers.VirtEngine.Market.OnBidClosed(ctx, bid)
		if err != nil {
			return err
		}

		if lease, ok := up.Keepers.VirtEngine.Market.GetLease(ctx, bid.ID.LeaseID()); ok {
			// OnGroupClosed is callable by x/deployment only so only reason is owner
			err = up.Keepers.VirtEngine.Market.OnLeaseClosed(ctx, lease, mv1.LeaseClosed, mv1.LeaseClosedReasonOwner)
			if err != nil {
				return err
			}
		}
	...
	var err error
	up.Keepers.VirtEngine.Market.WithOrdersForGroup(ctx, id, mtypes.OrderActive, func(order mtypes.Order) bool {
		err = up.Keepers.VirtEngine.Market.OnOrderClosed(ctx, order)
		if err != nil {
			return true
		}

		up.Keepers.VirtEngine.Market.WithBidsForOrder(ctx, order.ID, mtypes.BidOpen, func(bid mtypes.Bid) bool {
		...
		up.Keepers.VirtEngine.Market.WithBidsForOrder(ctx, order.ID, mtypes.BidActive, func(bid mtypes.Bid) bool {
			err = processClose(ctx, bid)
			return err != nil
		})
...
func (up *upgrade) OnEscrowPaymentClosed(ctx sdk.Context, obj etypes.Payment) error {
	...
	bid, ok := up.Keepers.VirtEngine.Market.GetBid(ctx, id.BidID())
	if !ok {
		return nil
	}
	...
	order, ok := up.Keepers.VirtEngine.Market.GetOrder(ctx, id.OrderID())
	if !ok {
		return mv1.ErrOrderNotFound
	}

	lease, ok := up.Keepers.VirtEngine.Market.GetLease(ctx, id)
	if !ok {
	...
	if obj.State.State == etypes.StateOverdrawn {
		err = up.Keepers.VirtEngine.Market.OnLeaseClosed(ctx, lease, mv1.LeaseInsufficientFunds, mv1.LeaseClosedReasonInsufficientFunds)
		if err != nil {
			return err
		}
	} else {
		err = up.Keepers.VirtEngine.Market.OnLeaseClosed(ctx, lease, mv1.LeaseClosed, mv1.LeaseClosedReasonUnspecified)
		if err != nil {
			return err
		}
	}
