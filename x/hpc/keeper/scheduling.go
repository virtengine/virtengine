...
func (k Keeper) UpdateNodeMetadata(ctx sdk.Context, node *types.NodeMetadata) error {
	...
	cluster, exists := k.GetCluster(ctx, node.ClusterID)
	if !exists {
		return types.ErrClusterNotFound
	}

	// Verify ownership
	if cluster.ProviderAddress != node.ProviderAddress {
	...
	if len(node.LatencyMeasurements) > 0 {
		var totalLatency int64
		for _, m := range node.LatencyMeasurements {
			totalLatency += m.LatencyMs
		}
		node.AvgLatencyMs = totalLatency / int64(len(node.LatencyMeasurements))
	}
...
// GetNodesByCluster retrieves nodes by cluster
