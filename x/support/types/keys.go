...
func EncodeSupportQueueTime(unix int64) []byte {
	...
	binary.BigEndian.PutUint64(bz, uint64(unix)^(uint64(1)<<63)) /* #nosec G115 -- signed timestamps are deliberately biased for sorting */ //nolint:gosec
	return bz
}
...
func DecodeSupportQueueTime(bz []byte) (int64, error) {
	...
	return int64(binary.BigEndian.Uint64(bz) ^ (uint64(1) << 63)), nil /* #nosec G115 -- reverse of EncodeSupportQueueTime biasing */ //nolint:gosec
}
