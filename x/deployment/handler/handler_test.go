...
func TestFundedDeployment(t *testing.T) {
	...
	suite.PrepareMocks(func(ts *state.TestSuite) {
		...
		bkeeper.
			...
			Return(nil).Once().
			On("SendCoinsFromModuleToAccount", mock.Anything, emodule.ModuleName, mock.Anything, sdk.NewCoins(testutil.VECoin(t, 490_000))).
			Return(nil).Once()
	})
	...
	suite.PrepareMocks(func(ts *state.TestSuite) {
		bkeeper := ts.BankKeeper()
		bkeeper.
			On("SendCoinsFromModuleToAccount", mock.Anything, emodule.ModuleName, owner, sdk.NewCoins(testutil.VECoin(t, 500_000))).
			Return(nil).Once().
			On("SendCoinsFromModuleToAccount", mock.Anything, emodule.ModuleName, suite.granter, sdk.NewCoins(testutil.VECoin(t, 500_000))).
			Return(nil).Once().
			On("SendCoinsFromModuleToAccount", mock.Anything, emodule.ModuleName, rndDepositor, sdk.NewCoins(testutil.VECoin(t, 500_000))).
			Return(nil).Once().
			On("SendCoinsFromModuleToAccount", mock.Anything, emodule.ModuleName, providerAddr, sdk.NewCoins(testutil.VECoin(t, 500_000))).
			Return(nil).Once()
	})
	...
	group.GroupSpec.Resources = v1beta4.ResourceUnits{
		{
			...
			Count:     1,
			Price:     testutil.VEDecCoinRandom(st.t),
		},
	}
