...
func (k Keeper) findActiveFinancialCaseForSubject(ctx sdk.Context, subject types.FinancialSubject) (string, bool, error) {
	...
	aliases := []struct{ kind, value string }{{"order", subject.OrderId}, {"invoice", subject.InvoiceId}, {"usage", subject.UsageId}, {"job", subject.HpcJobId}, {"escrow", subject.EscrowId}}
	owner := ""
...
func (k Keeper) addFinancialClaim(ctx sdk.Context, caseID string, claim types.FinancialClaim) (*types.FinancialCase, *types.FinancialClaim, bool, error) {
	...
	if claim.Claimant != financialCase.Claimant && claim.Claimant != financialCase.Respondent && claim.SourceModule == "settlement" {
...
func (k Keeper) AppealFinancialCase(ctx sdk.Context, caseID, appellant string, evidenceHash []byte, encryptedReference string, idempotency []byte) (*types.FinancialAppeal, bool, error) {
	...
	err := k.updateFinancialCase(ctx, caseID, func(financialCase *types.FinancialCase) error {
		...
		if len(evidenceHash) != financialHashSize || len(idempotency) == 0 {
		...
		if len(encryptedReference) > int(maxRef) {
		...
		for i := range financialCase.Appeals {
			if financialCase.Appeals[i].AppealId == appealID {
				result, duplicate = financialCase.Appeals[i], true
				return nil
			}
		}
		result = types.FinancialAppeal{AppealId: appealID, Appellant: appellant, EvidenceHash: append([]byte(nil), evidenceHash...), EncryptedReference: encryptedReference, CreatedHeight: ctx.BlockHeight(), CreatedAt: ctx.BlockTime().Unix()}
	...
	if !duplicate {
		_ = ctx.EventManager().EmitTypedEvent(&settlementv1.EventFinancialCaseAppealed{CaseId: caseID, AppealId: result.AppealId})
	}
...
func (k Keeper) ValidateFinancialCaseInvariants(ctx sdk.Context) []string {
	...
	if err := k.WithFinancialCases(ctx, func(financialCase types.FinancialCase) bool {
		...
		if types.IsActiveFinancialCaseStatus(financialCase.Status) {
			...
			if holds == 0 || holds != financialCase.ActiveHoldCount {
...
func (k Keeper) MigrateFinancialCases(ctx sdk.Context) (FinancialCaseMigrationReport, error) {
	...
	k.WithPayouts(ctx, func(payout types.PayoutRecord) bool {
		...
		request := FinancialCaseOpenRequest{
			...
			IdempotencyKey: []byte("migration/payout/" + payout.PayoutID + "/" + payout.DisputeID),
			Claim:          types.FinancialClaim{ClaimType: types.FinancialClaimTypeMigration, Claimant: payout.Customer, SourceModule: "settlement", SourceReference: payout.DisputeID, EvidenceHash: hashFinancialMigrationReference("payout", payout.PayoutID, payout.DisputeID), EncryptedReference: "migration://settlement/payout/" + payout.PayoutID},
		}
	...
	k.WithEscrowsByState(ctx, types.EscrowStateDisputed, func(escrow types.EscrowAccount) bool {
		...
		request := FinancialCaseOpenRequest{
			...
			IdempotencyKey: []byte("migration/escrow/" + escrow.EscrowID),
			Claim:          types.FinancialClaim{ClaimType: types.FinancialClaimTypeMigration, Claimant: escrow.Depositor, SourceModule: "settlement", SourceReference: escrow.EscrowID, EvidenceHash: hashFinancialMigrationReference("escrow", escrow.EscrowID), EncryptedReference: "migration://settlement/escrow/" + escrow.EscrowID},
		}
...
func (k Keeper) deriveFinancialExposure(ctx sdk.Context, subject types.FinancialSubject) (types.FinancialExposure, error) {
	...
	if exposure.PayoutAmount.IsZero() && exposure.EscrowAmount.IsZero() {
...
func (k Keeper) applyFinancialCaseEffects(ctx sdk.Context, financialCase *types.FinancialCase) error {
	...
	effects := []struct {
		...
		{"provider", types.FinancialEffectPayout, allocation.Provider, financialCase.Respondent},
		{"customer", types.FinancialEffectEscrow, allocation.Customer, financialCase.Claimant},
		{"platform", types.FinancialEffectPayout, allocation.Platform, ""},
		{"slash-witness", types.FinancialEffectReputation, allocation.SlashWitness, allocation.SlashWitnessRecipient},
	}
	for _, spec := range effects {
		...
		if !spec.amount.IsZero() && spec.address != "" {
			address, err := sdk.AccAddressFromBech32(spec.address)
			if err != nil {
				return types.ErrFinancialCaseEffect.Wrap(err.Error())
			}
			if err := k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleAccountName, address, spec.amount); err != nil {
		...
		financialCase.Effects[idx].Status, financialCase.Effects[idx].AppliedHeight, financialCase.Effects[idx].AppliedAt = types.FinancialEffectStatusApplied, ctx.BlockHeight(), ctx.BlockTime().Unix()
	...
	if financialCase.Exposure.PayoutId != "" {
		...
		if allocation.Provider.Equal(payout.GrossAmount) {
...
func (k Keeper) validateFinancialCase(ctx sdk.Context, financialCase types.FinancialCase) error {
	...
	sequence := uint64(1)
	for _, transition := range financialCase.Transitions {
