...
func (s MilestoneSet) Validate() error {
	...
	for i := uint32(1); i <= uint32(len(s)); i++ { // #nosec G115 -- len(s) is an in-memory slice length far below 2^32; sequences are uint32-typed and deduped in the validation loop above
		if !seenSeq[i] {
