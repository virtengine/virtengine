...
import (
	...
	dhandler "github.com/virtengine/virtengine/x/deployment/handler"
...
func TestMarketFullFlowCloseLease(t *testing.T) {
	...
	suite.PrepareMocks(func(ts *state.TestSuite) {
		...
		bkeeper.
			...
			Return(nil).Once().
			On("SendCoinsFromModuleToAccount", mock.Anything, emodule.ModuleName, providerAddr, sdk.NewCoins(testutil.VECoin(t, 490_000))).
			Return(nil).Once()
	})
	...
	suite.PrepareMocks(func(ts *state.TestSuite) {
		...
		bkeeper.
			...
			Return(nil).Once().
			On("SendCoinsFromModuleToAccount", mock.Anything, emodule.ModuleName, providerAddr, sdk.NewCoins(testutil.VECoin(t, 98))).
			Return(nil).Once()
	})
