...
func (tx *memoryIssuerLinkTransaction) InsertPending() (IssuerLinkRecord, error) {
	...
	record := IssuerLinkRecord{
		...
		CreatedAtUnix: int64(tx.coordinate), UpdatedAtUnix: int64(tx.coordinate), // #nosec G115 -- the fixture coordinate is a non-negative monotonic counter that fits in int64
...
func (f *MemoryAtomicIssuerLinkFixture) Transition(ctx context.Context, domain NullifierDomain, nullifier string, action TransitionAction) error {
	...
	next := previous
	next.UpdatedAtCoordinate, next.UpdatedAtUnix = f.coordinate, int64(f.coordinate) // #nosec G115 -- the fixture coordinate is a non-negative monotonic counter that fits in int64
	switch action.Kind {
...
func (f *MemoryAtomicIssuerLinkFixture) ActivateRotation(ctx context.Context, domain NullifierDomain, newNullifier, supersessionCommitment, notificationCommitment string) error {
	...
	oldNext.Status, oldNext.UpdatedAtCoordinate, oldNext.UpdatedAtUnix = StatusSuperseded, f.coordinate, int64(f.coordinate) // #nosec G115 -- the fixture coordinate is a non-negative monotonic counter that fits in int64
	oldNext.SupersessionCommitment, oldNext.NotificationCommitment = supersessionCommitment, notificationCommitment
	newNext.Status, newNext.UpdatedAtCoordinate, newNext.UpdatedAtUnix = StatusActive, f.coordinate, int64(f.coordinate) // #nosec G115 -- the fixture coordinate is a non-negative monotonic counter that fits in int64
...
func (f *MemoryAtomicIssuerLinkFixture) ApplyRetention(ctx context.Context, domain NullifierDomain, nullifier string, authorization VerifiedRetentionAuthorization) error {
	...
	record.UpdatedAtCoordinate, record.UpdatedAtUnix = f.coordinate, int64(f.coordinate) // #nosec G115 -- the fixture coordinate is a non-negative monotonic counter that fits in int64
	switch claim.Action {
	...
	case RetentionDeleteAction:
		...
		tombstone := DeletionTombstone{Version: Version1, RecordCommitment: commitment, DeletionAuthorizationDigest: authorization.digest, DeletedAtUnix: int64(f.coordinate), DeletedAtCoordinate: f.coordinate} // #nosec G115 -- the fixture coordinate is a non-negative monotonic counter that fits in int64
