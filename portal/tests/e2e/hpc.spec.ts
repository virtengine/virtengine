...
test.describe('HPC Templates', () => {
  ...
  test('should display template categories', async ({ page }) => {
    ...
    // Use exact: true to avoid matching "Connect Wallet" button for /all/i pattern
  ...
  test('should display template cards', async ({ page }) => {
    ...
    // Should have template cards - use heading role to be specific
    await expect(page.getByRole('heading', { name: /pytorch training/i })).toBeVisible();
    await expect(page.getByRole('heading', { name: /tensorflow/i })).toBeVisible();
  });
});
