...
function init() {
	...
	for val in $(jq -c '.validators[]' <<<"$config"); do
		...
		valdir=$validators_dir/.virtengine${cnt}
		...
		if [[ $cnt -eq 0 ]]; then
			...
			cp "$validators_dir/.virtengine0/cosmovisor/genesis/bin/virtengine" "$genesis_bin/virtengine"
			cp "$validators_dir/.virtengine0/cosmovisor/upgrades/$UPGRADE_TO/bin/virtengine" "$upgrade_bin/virtengine"
		fi
		...
		VIRTENGINE=$genesis_bin/virtengine

		genesis_file=${valdir}/config/genesis.json
		rm -f "$genesis_file"

		$VIRTENGINE genesis init --home "$valdir" "$(jq -rc '.moniker' <<<"$val")" >/dev/null 2>&1
...
function prepare_state() {
	...
	rvaldir=$validators_dir/.virtengine0

	echo "testnetifying state"
	$VIRTENGINE testnetify --home="$rvaldir" --testnet-rootdir="$validators_dir" --testnet-config="${STATE_CONFIG}" --yes
	...
	if [[ $MAX_VALIDATORS -gt 1 ]]; then
			...
			if [[ $cnt -gt 0 ]]; then
				if [[ $($semver compare "$virtengineversion" v1.0.0-rc0) -ge 0 ]]; then
					...
					$VIRTENGINE tx staking create-validator \
						...
						--pubkey="$($VIRTENGINE tendermint show-validator --home="$valdir")" \
						--from="validator$cnt" \
						--yes
				fi
			fi
		...
		local srcval
		srcval="${validators_dir}/.virtengine0"
...
function clean() {
	...
	for val in $(jq -c '.validators[]' <<<"$config"); do
		local valdir=$validators_dir/.virtengine${cnt}
		local cosmovisor_dir=$valdir/cosmovisor

		rm -rf "$validators_dir/logs/.virtengine${cnt}-stderr.log"
		rm -rf "$validators_dir/logs/.virtengine${cnt}-stdout.log"
...
function import_keys() {
	...
	valdir=$validators_dir/.virtengine0
	cosmovisor_dir=$valdir/cosmovisor
	genesis_bin=$cosmovisor_dir/genesis/bin
	...
	jq -c '.mnemonics[]' <<<"$config" | while read -r mnemonic; do
		jq -c '.keys[]' <<<"$mnemonic" | while read -r key; do
			jq -rc '.phrase' <<<"$mnemonic" | $VIRTENGINE --home="$valdir" --keyring-backend=test keys add "$(jq -rc '.name' <<<"$key")" --recover --index "$(jq -rc '.index' <<<"$key")"
		done
	done
}
