...
// InitVirtEngineAppForTestnet is broken down into two sections:
// Required Changes: Changes that, if not made, will cause the testnet to halt or panic
// Optional Changes: Changes to customize the testnet to one's liking (lower vote times, fund accounts, etc)
func InitVirtEngineAppForTestnet(
	app *VirtEngineApp,
	db dbm.DB,
	tcfg *TestnetConfig,
) *VirtEngineApp {
	...
	for _, val := range tcfg.Validators {
		...
		bech32Addr, err := bech32.ConvertAndEncode("virtenginevaloper", bz)
		if err != nil {
			panic(err.Error())
		}
