...
import (
	...
	benchkeeper "pkg.akt.dev/node/x/benchmark/keeper"
	benchtypes "pkg.akt.dev/node/x/benchmark/types"
	ckeeper "pkg.akt.dev/node/x/cert/keeper"
	configkeeper "pkg.akt.dev/node/x/config/keeper"
	configtypes "pkg.akt.dev/node/x/config/types"
	delegationkeeper "pkg.akt.dev/node/x/delegation/keeper"
	delegationtypes "pkg.akt.dev/node/x/delegation/types"
	dkeeper "pkg.akt.dev/node/x/deployment/keeper"
	enclavekeeper "pkg.akt.dev/node/x/enclave/keeper"
	enclavetypes "pkg.akt.dev/node/x/enclave/types"
	encryptionkeeper "pkg.akt.dev/node/x/encryption/keeper"
	encryptiontypes "pkg.akt.dev/node/x/encryption/types"
	ekeeper "pkg.akt.dev/node/x/escrow/keeper"
	fraudkeeper "pkg.akt.dev/node/x/fraud/keeper"
	fraudtypes "pkg.akt.dev/node/x/fraud/types"
	hpckeeper "pkg.akt.dev/node/x/hpc/keeper"
	hpctypes "pkg.akt.dev/node/x/hpc/types"
	...
	reviewkeeper "pkg.akt.dev/node/x/review/keeper"
	reviewtypes "pkg.akt.dev/node/x/review/types"
	roleskeeper "pkg.akt.dev/node/x/roles/keeper"
	rolestypes "pkg.akt.dev/node/x/roles/types"
	settlementkeeper "pkg.akt.dev/node/x/settlement/keeper"
	settlementtypes "pkg.akt.dev/node/x/settlement/types"
	virtstakingkeeper "pkg.akt.dev/node/x/staking/keeper"
	virtstakingtypes "pkg.akt.dev/node/x/staking/types"
	tkeeper "pkg.akt.dev/node/x/take/keeper"
	veidkeeper "pkg.akt.dev/node/x/veid/keeper"
	veidtypes "pkg.akt.dev/node/x/veid/types"
)

const (
	AccountAddressPrefix = "virtengine"
)
...
func (app *App) InitNormalKeepers(
	...
	app.Keepers.VirtEngine.Take = tkeeper.NewKeeper(
		cdc,
		app.keys[ttypes.StoreKey],
		authtypes.NewModuleAddress(govtypes.ModuleName).String(),
	)

	app.Keepers.VirtEngine.Escrow = ekeeper.NewKeeper(
		cdc,
	...
	app.Keepers.VirtEngine.Deployment = dkeeper.NewKeeper(
		cdc,
	...
	app.Keepers.VirtEngine.Market = mkeeper.NewKeeper(
		cdc,
	...
	app.Keepers.VirtEngine.Provider = pkeeper.NewKeeper(
		cdc,
		app.keys[ptypes.StoreKey],
	)

	app.Keepers.VirtEngine.Audit = akeeper.NewKeeper(
		cdc,
		app.keys[atypes.StoreKey],
	)

	app.Keepers.VirtEngine.Cert = ckeeper.NewKeeper(
		cdc,
	...
	// Simple keepers (no VirtEngine dependencies)
	app.Keepers.VirtEngine.Encryption = encryptionkeeper.NewKeeper(
		cdc,
		app.keys[encryptiontypes.StoreKey],
		authtypes.NewModuleAddress(govtypes.ModuleName).String(),
	)

	app.Keepers.VirtEngine.Roles = roleskeeper.NewKeeper(
		cdc,
		app.keys[rolestypes.StoreKey],
		authtypes.NewModuleAddress(govtypes.ModuleName).String(),
	)
	...
	app.Keepers.VirtEngine.Config = configkeeper.NewKeeper(
		cdc,
		app.keys[configtypes.StoreKey],
		authtypes.NewModuleAddress(govtypes.ModuleName).String(),
	)
	...
	app.Keepers.VirtEngine.Benchmark = benchkeeper.NewKeeper(
		cdc,
		app.keys[benchtypes.StoreKey],
		app.Keepers.VirtEngine.Provider,
		app.Keepers.VirtEngine.Roles,
		authtypes.NewModuleAddress(govtypes.ModuleName).String(),
	)

	app.Keepers.VirtEngine.Settlement = settlementkeeper.NewKeeper(
		cdc,
	...
	app.Keepers.VirtEngine.Delegation = delegationkeeper.NewKeeper(
		cdc,
		app.keys[delegationtypes.StoreKey],
		app.Keepers.Cosmos.Bank,
	...
	app.Keepers.VirtEngine.VirtStaking = virtstakingkeeper.NewKeeper(
		cdc,
		app.keys[virtstakingtypes.StoreKey],
		app.Keepers.Cosmos.Bank,
		app.Keepers.VirtEngine.VEID,
