...
func simulateFromSeedFunc(t *testing.T, newApp *virtengine.VirtEngineApp, config sdksim.Config) (bool, simulation.Params, error) {
	return simulation.SimulateFromSeed(
		t,
		os.Stdout,
		newApp.BaseApp,
		simtestutil.AppStateFn(newApp.AppCodec(), newApp.SimulationManager(), virtengine.NewDefaultGenesisState(newApp.AppCodec())),
...
func TestFullAppSimulation(t *testing.T) {
	...
	virtengine.ModuleBasics().RegisterInterfaces(encodingConfig.InterfaceRegistry)
	...
	app1 := virtengine.NewApp(logger, db, nil, true, sim.FlagPeriodValue, map[int64]bool{}, encodingConfig, appOpts, fauxMerkleModeOpt, baseapp.SetChainID(AppChainID))

	require.Equal(t, "virtengine", app1.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	appA := virtengine.NewApp(logger, db, nil, true, sim.FlagPeriodValue, map[int64]bool{}, encodingConfig, appOpts, fauxMerkleModeOpt, baseapp.SetChainID(AppChainID))
	require.Equal(t, virtengine.AppName, appA.Name())
	...
	appB := virtengine.NewApp(logger, newDB, nil, true, sim.FlagPeriodValue, map[int64]bool{}, encodingConfig, appOpts, fauxMerkleModeOpt, baseapp.SetChainID(AppChainID))
	require.Equal(t, virtengine.AppName, appB.Name())

	var genesisState virtengine.GenesisState
...
func TestAppSimulationAfterImport(t *testing.T) {
	...
	app := virtengine.NewApp(logger, db, nil, true, sim.FlagPeriodValue, map[int64]bool{}, encodingConfig, appOpts, fauxMerkleModeOpt, baseapp.SetChainID(AppChainID))
	require.Equal(t, virtengine.AppName, app.Name())
	...
	newApp := virtengine.NewApp(log.NewNopLogger(), newDB, nil, true, sim.FlagPeriodValue, map[int64]bool{}, encodingConfig, appOpts, fauxMerkleModeOpt, baseapp.SetChainID(AppChainID))
	require.Equal(t, virtengine.AppName, newApp.Name())
...
func TestAppStateDeterminism(t *testing.T) {
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			...
			app := virtengine.NewApp(logger, db, nil, true, sim.FlagPeriodValue, map[int64]bool{}, encodingConfig, appOpts, interBlockCacheOpt(), baseapp.SetChainID(AppChainID))
